...
describe('workers/transpiler', function () {
  ...
  var transpiler = (0, _proxyquire2.default)('workers/transpiler', {
  ...
  describe('transpile()', function () {
    ...
    it('reports status to the worker and updates internal status', function (done) {
      ...
      var worker = { postMessage: _sinon2.default.spy() };

      transpiler.transpile(store, worker);

      (0, _chai.expect)(worker.postMessage).to.have.been.calledWith((0, _workers.busy)(_workers.WORKER_TRANSPILER));
      (0, _chai.expect)(worker.postMessage).to.have.been.calledWith((0, _workers.ready)(_workers.WORKER_TRANSPILER));
    });
  });
});
